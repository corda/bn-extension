...
abstract class MembershipManagementFlow<T> : FlowLogic<T>() {
    ...
    @Suspendable
    protected fun authorise(networkId: String, BNService: BNService, authorisationMethod: (MembershipState) -> Boolean): StateAndRef<MembershipState> {
    ...
    protected fun syncMembershipsParticipants(
        ...
        val allGroups = BNService.getAllBusinessNetworkGroups(networkId).map { it.state.data }
